    ...
    if json_query:
        ...
        if 'result' in json_query and 'movies' in json_query['result']:
            for movie in json_query['result']['movies']:
                ...
                liz.getVideoInfoTag(type="Video", infoLabels={"Title": movie['title'], # liz.setInfo(type="Video", infoLabels={"Title": movie['title'],
                ...
                liz.setProperty("setResumePoint", str(movie['resume']['position'])) # liz.setProperty("resumetime", str(movie['resume']['position']))
                liz.setProperty("setResumePoint", str(movie['resume']['total'])) # liz.setProperty("totaltime", str(movie['resume']['total']))
                ...
                for key, value in list(movie['streamdetails'].items()):
                    for stream in value:
                        if 'video' in key:
                            hasVideo = True
                        liz.getVideoInfoTag(key, stream) # liz.addStreamInfo(key, stream)

                # if duration wasnt in the streaminfo try adding the scraped one
                if not hasVideo:
                    stream = {'duration': movie['runtime']}
                    liz.getVideoInfoTag("video", stream) # liz.addStreamInfo("video", stream)
        ...
        if "result" in json_query and 'tvshows' in json_query['result']:
            ...
            for tvshow in json_query['result']['tvshows']:
                ...
                if json_query2:
                    ...
                    if "result" in json_query2 and json_query2['result'] is not None and 'episodes' in json_query2['result'] and len(json_query2['result']['episode'])>0:
                        ...
                        liz.getVideoInfoTag(type="Video", infoLabels={"Title": episode['title'], # liz.setInfo(type="Video", infoLabels={"Title": episode['title'],
                        ...
                        liz.setProperty("setResumePoint", str(episode['resume']['position'])) # liz.setProperty("resumetime", str(episode['resume']['position']))
                        liz.setProperty("setResumePoint", str(episode['resume']['total'])) # liz.setProperty("totaltime", str(episode['resume']['total']))
                        liz.setProperty("type", ADDON_LANGUAGE(list_type))
    ...
    if json_query:
        ...
        if 'result' in json_query and 'songs' in json_query['result']:
            for song in json_query['result']['songs']:
                ...
                liz.getVideoInfoTag(type="Music", infoLabels={"Title": song['title']}) # liz.setInfo(type="Music", infoLabels={"Title": song['title']})
                if song['artist']:
                    liz.getVideoInfoTag(type="Music", infoLabels={"Artist": song['artist'][0], # liz.setInfo(type="Music", infoLabels={"Artist": song['artist'][0],
    ...
    if json_query:
        ...
        if 'result' in json_query and 'albums' in json_query['result']:
            ...
            for album in json_query['result']['albums']:
                ...
                liz.getVideoInfoTag(type="Music", infoLabels={"Title": album['title']}) # liz.setInfo(type="Music", infoLabels={"Title": album['title']})
                if album['artist']:
                    liz.getVideoInfoTag(type="Music", infoLabels={"Artist": album['artist'][0], # liz.setInfo(type="Music", infoLabels={"Artist": album['artist'][0],
    ...
    if json_query:
        ...
        if 'result' in json_query and 'musicvideos' in json_query['result']:
            for musicvideo in json_query['result']['musicvideos']:
                ...
                liz.getVideoInfoTag(type="Video", infoLabels={"Title": musicvideo['title'], # liz.setInfo(type="Video", infoLabels={"Title": musicvideo['title'],
                ...
                liz.setProperty("setResumePoint", str(musicvideo['resume']['position'])) # liz.setProperty("resumetime", str(musicvideo['resume']['position']))
                liz.setProperty("setResumePoint", str(musicvideo['resume']['total'])) # liz.setProperty("totaltime", str(musicvideo['resume']['total']))
                liz.setProperty("type", ADDON_LANGUAGE(list_type))
